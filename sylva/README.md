...
Integration of Python-based test cases from https://github.com/petorre/container-experience-kits/tree/master/validation/sylva-validation/stack-validation into [Xtesting framework](https://github.com/opnfv/functest-xtesting).
...
Check [build/image/etc/xtesting/testcases.yaml](./build/image/etc/xtesting/testcases.yaml) for test cases. Further test cases configuration details are in [build/image/stack-validation/config](./build/image/stack-validation/config.json) (created during build process).

If pods on Kubernetes worker nodes should use http_proxy to reach outside HTTP URLs, configure it in [build/image/stack-validation/k8s/proxy.yaml#L12](./build/image/stack-validation/k8s/proxy.yaml#L12).

## Building and pushing Docker image

Use script [build/build.sh](./build/build.sh) or do similar.
...
cd build/image/stack-validation
python validate.py
```
