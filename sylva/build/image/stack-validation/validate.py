...
class Validate:
    ...
    def load_config(self, s):
        ...
        self.ns = s["namespace"]
        self.proxy = s["deployFiles"]["proxy"]
        self.runsh = s["deployFiles"]["runsh"]
    ...
    def run(self, test):
        ...
        if test is None:
            ...
            self.create_from_yaml(self.proxy, False)
            self.create_from_yaml(self.runsh, False)
            if self.check_empty_namespace():
                # schedule all daemonset even if some test cases have
                #   skipFromAll = true
                self.create_from_yaml(self.multi, False)
                self.create_from_yaml(self.huge2mi, False)
                self.create_from_yaml(self.huge1gi, False)
                self.create_from_yaml(self.reserve, False)
                self.create_from_yaml(self.tunedrt, True)
        ...
        if test in tests_with_multi_daemonset:
            ...
            self.create_from_yaml(self.runsh, False)
            if self.check_empty_namespace():
                self.create_from_yaml(self.multi, True)
        ...
        if test == "validateHugepages":
            ...
            if self.check_empty_namespace():
                self.create_from_yaml(self.huge2mi, False)
                self.create_from_yaml(self.huge1gi, True)
                self.validate_hugepages()
        if test == "validateSMT":
            self.validate_smt()
        ...
        if test == "validateSystemResourceReservation":
            ...
            self.create_from_yaml(self.runsh, False)
            if self.check_empty_namespace():
                self.create_from_yaml(self.reserve, True)
                self.validate_system_resource_reservation()
        if test == "validateRT":
    ...
    def create_from_yaml(self, name, with_sleep):
        """
        Creates DaemonSet, ConfigMap or Secret.
        """
        ...
        try:
            ...
            self.resj["error"] = ( "Error creating DaemonSet or ConfigMap "
                f"{name}: {e}" )
